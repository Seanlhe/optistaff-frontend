...
describe('usePreferences Hooks - Error Scenarios and Failure Tests', () => {
  ...
  describe('Database Error Scenarios', () => {
    ...
    test('create_default_preferences handles RPC function error', async () => {
      ...
      mockSupabase.rpc.mockResolvedValue({
        data: null,
        error: { message: 'Function create_default_preferences does not exist' }
      })
      ...
      // Assert
      expect(result.current.preferences).toBeNull()
      expect(result.current.error).toBe('Function create_default_preferences does not exist')
    })
    ...
    test('upsert_user_preferences handles constraint violation', async () => {
      ...
      mockSupabase.rpc.mockResolvedValue({
        data: null,
        error: { 
          message: 'new row for relation "preferences" violates check constraint "preferences_max_hours_per_week_check"',
          code: '23514'
        }
      })
      ...
      let saveResult: boolean = true
      await act(async () => {
        saveResult = await result.current.savePreferences(invalidPreferencesData)
      })
  ...
  describe('Location Service Error Scenarios', () => {
    test('usePreferencesLocation handles get_user_location RPC error', async () => {
      ...
      expect(result.current.error).toBe('Failed to load location data: Function get_user_location does not exist')
    })
    ...
    test('geocodeHomeLocation handles geocoding service failure', async () => {
      ...
      const mockGeocodeAddress = vi.fn().mockRejectedValue(new Error('Geocoding service unavailable'))

      mockUseLocationGeocoding.mockReturnValue({
        geocodeAddress: mockGeocodeAddress,
        reverseGeocode: vi.fn(),
        loading: false,
        error: { type: 'NETWORK_ERROR', message: 'Geocoding service unavailable' }
      })
    ...
    test('geocodeHomeLocation handles missing postal code', async () => {
      ...
      // Assert
      expect(coordinates).toBeNull()
      expect(result.current.error).toContain('No postal code available for geocoding')
    })
  })
  ...
  describe('Validation Error Scenarios', () => {
    test('upsert_user_preferences handles validation errors from database function', async () => {
      ...
      // Assert
      expect(saveResult).toBe(false)
      expect(result.current.error).toBe('Please select at least one preferred job type, One or more selected job types are invalid or inactive')
    })
    ...
    test('usePreferencesForm handles client-side validation failure', async () => {
      ...
      // Assert
      expect(saveResult).toBe(false)
      expect(result.current.error).toContain('Pay rate must be greater than or equal to 0')
    })
  })
  ...
  describe('Network and Connectivity Error Scenarios', () => {
    ...
    test('handles intermittent network failure during save operation', async () => {
      ...
      await act(async () => {
        secondSaveResult = await result.current.savePreferences(preferencesData)
      })
  ...
  describe('Edge Cases and Boundary Conditions', () => {
    ...
    test('handles concurrent save operations', async () => {
      ...
      expect(result.current.preferences?.min_pay_rate).toBeGreaterThanOrEqual(20)
    })
  })
})