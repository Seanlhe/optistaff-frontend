...
describe("ClientRoster Page", () => {
  ...
  describe("Shift Selection and Details", () => {
    it("opens shift details when shift is clicked", async () => {
      render(<ClientRoster />);
  ...
  describe("Shift Cancellation", () => {
    it("cancels shift when cancel button clicked", async () => {
      render(<ClientRoster />);
    ...
    it("handles cancellation errors gracefully", async () => {
      const consoleErrorSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});
      mockUpdateShiftStatus.mockRejectedValueOnce(
        new Error("Cancellation failed")
      );

      render(<ClientRoster />);

      // Open shift details and cancel
      const shiftButton = screen.getByTestId("shift-shift-1");
      fireEvent.click(shiftButton);
