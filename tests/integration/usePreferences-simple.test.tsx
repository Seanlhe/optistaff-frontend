...
describe("Preferences Integration Tests - Direct Database Operations", () => {
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      const { data: jobSeeker, error: jobSeekerError } = await testSupabaseAdmin
        .from("job_seekers")
        .insert({
          user_id: testUserId,
          first_name: "Test",
  ...
  describe("Location Data Integration", () => {
    ...
    test("validates Singapore coordinates bounds", async () => {
      ...
      const isValidSingapore =
        lat >= 1.229 && lat <= 1.4784 && lng >= 103.6 && lng <= 104.012;
      ...
      const [invalidLat, invalidLng] = invalidCoordinates
        .split(",")
        .map(Number);
      const isInvalidSingapore =
        invalidLat >= 1.229 &&
        invalidLat <= 1.4784 &&
        invalidLng >= 103.6 &&
        invalidLng <= 104.012;
  ...
  describe("Preferences Business Logic", () => {
    ...
    test("enforces positive pay rate constraint", async () => {
      ...
      const { error } = await testSupabase.from("preferences").insert({
        user_id: testUserId,
