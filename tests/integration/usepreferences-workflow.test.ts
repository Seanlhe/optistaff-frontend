...
describe("usePreferences Database Functions - Integration Workflow Tests", () => {
  ...
  describe("Complete User Preferences Lifecycle", () => {
    ...
    test("existing user workflow: fetch → validate → update preferences", async () => {
      ...
      const { data: updateResult, error: updateError } = await testSupabase.rpc(
        "upsert_user_preferences",
        {
          p_target_user_id: jobSeeker.user_id,
          p_min_pay_rate: 20,
  ...
  describe("Location and Preferences Integration", () => {
    test("complete user setup: location + preferences workflow", async () => {
      ...
      const { data: locationData, error: locationError } =
        await testSupabase.rpc("get_user_location", {
          p_user_id: jobSeeker.user_id,
        });
      ...
      const { data: prefsResult, error: prefsError } = await testSupabase.rpc(
        "upsert_user_preferences",
        {
          p_target_user_id: jobSeeker.user_id,
          p_min_pay_rate: 18,
      ...
      const { data: finalLocation } = await testSupabase.rpc(
        "get_user_location",
        {
          p_user_id: jobSeeker.user_id,
        },
  ...
  describe("Error Handling and Recovery Workflows", () => {
    test("validation failure → retry with corrected data workflow", async () => {
      ...
      const { data: failResult, error: failError } = await testSupabase.rpc(
        "upsert_user_preferences",
        {
          p_target_user_id: jobSeeker.user_id,
          p_min_pay_rate: 20,
      ...
      expect(failResult[0].validation_errors).toContain(
      ...
      const { data: successResult, error: successError } =
        await testSupabase.rpc("upsert_user_preferences", {
          p_target_user_id: jobSeeker.user_id,
          p_min_pay_rate: 20,
          p_max_travel_km: 15,
    ...
    test("missing location → create preferences → add location later workflow", async () => {
      ...
      const { data: initialLocation, error: locationError1 } =
        await testSupabase.rpc("get_user_location", {
          p_user_id: jobSeeker.user_id,
        });
      ...
      const { data: updatedLocation, error: locationError2 } =
        await testSupabase.rpc("get_user_location", {
          p_user_id: jobSeeker.user_id,
        });
      ...
      expect(updatedLocation[0].coordinates_lng).toBe(103.852);
      ...
      const { data: locationAwarePrefs, error: updateError } =
        await testSupabase.rpc("upsert_user_preferences", {
          p_target_user_id: jobSeeker.user_id,
          p_min_pay_rate: 25,
  ...
  describe("Performance and Concurrency", () => {
    ...
    test("batch validation of multiple job names", async () => {
      ...
      const { data: isValid, error } = await testSupabase.rpc(
        "validate_job_names",
        {
          job_names: jobNames,
        },
