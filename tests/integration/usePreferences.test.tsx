  ...
  describe('savePreferences - Boundary Value Testing', () => {
    ...
    test('saves preferences with maximum valid values', async () => {
      ...
      await testSupabase.from('job_types').insert([
        { type_name: 'Server', category_id: category.category_id, description: 'Server', is_active: true },
        { type_name: 'Manager', category_id: category.category_id, description: 'Manager', is_active: true },
        { type_name: 'Cook', category_id: category.category_id, description: 'Cook', is_active: true }
      ])
      ...
      expect(result.current.preferences?.desired_roles).toEqual(['Server', 'Manager', 'Cook'])
  ...
  describe('updatePreferences - Decision Table Testing', () => {
    ...
    updateTestCases.forEach(({ description, update, expectedField, expectedValue }) => {
      test(description, async () => {
        ...
        expect(result.current.preferences?.[expectedField as keyof typeof result.current.preferences]).toEqual(expectedValue)
        expect(result.current.error).toBeNull()
      })
    })
  })
