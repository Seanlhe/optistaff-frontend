...
describe.skip('Preferences Database Functions - Unit Tests (Functions Not Implemented)', () => {
  ...
  describe('upsert_user_preferences function - Decision Table Testing', () => {
    ...
    testCases.forEach(({ description, existingPreferences, input, expectedSuccess }) => {
      test(description, async () => {
        ...
        if (expectedSuccess) {
          ...
          expect(savedPreferences.max_hours_per_week).toBe(input.max_hours_per_week)
          expect(savedPreferences.max_hours_per_shift).toBe(input.max_hours_per_shift)
          expect(savedPreferences.consider_lower_rate).toBe(input.consider_lower_rate)
        } else {
