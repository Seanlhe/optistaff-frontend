...
function UpdateForm({shift, onClose}: {shift: Shift, onClose: Function}){
    ...
    return <div id="upload-jobs-form" className="w-2/3">
            <form className="grid grid-cols-12 gap-x-4 gap-y-12 items-center">
                ...
                <CustomInputField disabled={disabled} className="col-span-6" name="job_title" title="Job Title" value = {formData.job_title} valid = {valid} error={shiftError.job_title} placeholder="Eg. Banquet Server" type="text" onChange={handleDataChange}/>
                <CustomSelect disabled={disabled} options={jobRoleOptions} className="col-span-6" name="job_type" title="Job Category" value = {formData.job_type} valid = {valid} error={shiftError.job_type} placeholder="Eg. Banquet Server" type="text" onInput={handleDataChange}/>
                <CustomTextArea disabled={disabled} className="col-span-12 h-[8rem]" name="job_description" title="Description" value = {formData.job_description != null? formData.job_description: ""} valid = {valid} error={shiftError.job_description} placeholder="Format into sections to improve readability. Give clear responsibilities and roles" onChange={handleDataChange}/>
                <CustomTextArea disabled={disabled} className="col-span-12 h-[8rem]" name="job_requirements" title="Requirements" value = {formData.job_requirements != null? formData.job_requirements: ""} valid = {valid} error={shiftError.job_requirements} placeholder="Clearly state any preparation required by staff. For example, attire or tools required."onChange={handleDataChange}/>
                <p className="pt-3 col-span-12 font-montserrat-b text-lg text-black">Time and Venue</p>
                <CustomInputField disabled={disabled} className="col-span-3"  name="date" title="Date" type="date" value = {format(formData.start_time, "yyyy-MM-dd")} valid={valid} error={formData.start_time <= new Date()? "Please choose a date after today.": null} onChange={handleDataChange}/>
                <CustomInputField disabled={disabled} className="col-span-3" name="start_time" title="Start Time" type="time" value = {format(formData.start_time, "HH:mm")} valid = {valid} error={shiftError.start_time} onChange={handleDataChange}/>
                <CustomInputField disabled={disabled} className="col-span-3" name="end_time" title="End Time" type="time" value = {format(formData.end_time, "HH:mm")} valid = {valid} error={shiftError.end_time} onChange={handleDataChange}/>
                <CustomInputField disabled={disabled} className="col-span-3" name="break_duration" title="Break (hrs)" value = {formData.break_duration? formData.break_duration : 0} type="number" valid = {valid}  error = {shiftError.break_duration} onChange={handleDataChange}/>
                <CustomInputField disabled={disabled} className="col-span-6" name="job_location" title="Address" type="text" value = {formData.job_location} valid = {valid} error = {shiftError.job_location} onChange={handleDataChange}/>
                <CustomInputField disabled={disabled} className="col-span-6" name="postal_code" title="Postal Code" type="text" value = {formData.postal_code} valid = {valid} error = {shiftError.postal_code} onChange={handleDataChange}/>
                <p className="pt-3 col-span-12 font-montserrat-b text-lg text-black">Staffing Requirements</p>
                <CustomInputField disabled={disabled} className="col-span-6"  placeholder="Eg. 7000" name="pay_rate" title="Pay Rate (/hr)" type="number" value = {formData.pay_rate} valid={valid} error = {shiftError.pay_rate} onChange={handleDataChange}/>
                <CustomInputField disabled={disabled} className="col-span-6" placeholder="Eg. 10"name="staff_needed" title="No. Pax" type="number" value = {formData.staff_needed} valid={valid} error = {shiftError.staff_needed} onChange={handleDataChange}/>
                <div id="upload-btns" className="col-span-12 flex flex-row gap-4 justify-end">
                    {!disabled && <button type="button" className="hover:cursor-pointer hover:opacity-80 w-full p-3 bg-primary-blue font-montserrat-smb text-white text-base rounded-lg" onClick={(e: React.MouseEvent<HTMLButtonElement>)=>handleSubmit(e)}>Update Job</button>}
