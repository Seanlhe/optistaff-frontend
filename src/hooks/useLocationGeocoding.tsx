...
export const useLocationGeocoding = (): LocationGeocodingHook => {
  ...
  const createError = (type: GeocodeErrorType, message: string, originalError?: any): GeocodeError => ({
    type,
    message,
    originalError
  });
  ...
  const setErrorStates = useCallback((type: LocationErrorType, originalError?: any, context?: string) => {
    ...
    const legacyType = type === 'NETWORK_ERROR' ? 'NETWORK_ERROR' :
                      type === 'API_UNAVAILABLE' ? 'API_KEY_ERROR' :
                      type === 'GEOCODING_FAILED' ? 'INVALID_ADDRESS' :
                      'UNKNOWN_ERROR';
    
    setError(createError(legacyType as GeocodeErrorType, locationErr.userMessage, originalError));
  }, []);
  ...
  const withRetry = async <T,>(
    ...
    for (let attempt = 0; attempt <= RETRY_CONFIG.maxRetries; attempt++) {
      try {
        ...
        if (err.message?.includes('REQUEST_DENIED') || err.message?.includes('API_KEY_NOT_CONFIGURED')) {
          throw err;
        }
        ...
        console.warn(`${operationName} attempt ${attempt + 1} failed, retrying in ${delay}ms:`, err);
        await sleep(delay);
      }
    }
  ...
  const geocodeAddress = useCallback(async (address: string): Promise<[number, number] | null> => {
    ...
    try {
      const result = await withRetry(async () => {
        ...
        const searchAddress = address.includes('Singapore') ? address : `${address}, Singapore`;
  ...
  const reverseGeocode = useCallback(async (coordinates: [number, number]): Promise<string | null> => {
    ...
    if (!isValidSingaporeCoordinates(lat, lng)) {
      setErrorStates('GEOCODING_FAILED', null, 'Coordinates outside Singapore bounds');
      return null;
    }
    ...
    try {
      ...
      if (err.message === 'API_KEY_NOT_CONFIGURED') {
        ...
        setErrorStates('GEOCODING_FAILED', err, 'No address found for coordinates');
      } else if (err.message?.includes('RATE_LIMIT_HTTP')) {
        setErrorStates('API_UNAVAILABLE', err, 'Rate limit exceeded');
      } else if (err.message?.includes('NETWORK_HTTP')) {
        setErrorStates('NETWORK_ERROR', err, 'HTTP error');
        ...
        setErrorStates('UNKNOWN_ERROR', err, 'Unexpected reverse geocoding error');
      }
