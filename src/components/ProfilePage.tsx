    ...
    return (
      <div className="bg-card-color p-6 rounded-xl border border-border">
        <div className="flex items-start">
          ...
          <div>
            <h3 className="text-sm font-medium text-red-dark">Error Loading Profile</h3>
    ...
    return (
      <div className="bg-card-color p-6 rounded-xl border border-border">
        <div className="text-center">
          <svg className="mx-auto h-12 w-12 text-secondary-text" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
          </svg>
          <h3 className="mt-2 text-sm font-medium text-primary-text">No Profile Data</h3>
    ...
    <div className="space-y-6">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="bg-secondary-bg p-4 rounded-lg text-xs text-secondary-text">
          <strong>Debug Info:</strong> User Role: {isJobSeeker() ? 'Job Seeker' : isClient() ? 'Client' : 'Unknown'}
        </div>
      )}
    </div>
  );
