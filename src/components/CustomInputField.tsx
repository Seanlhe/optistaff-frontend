...
export default function CustomInputField({title, name, type, placeholder, valid, error, className, onChange}: InputFieldProps){
    return <div className={`w-full h-15 flex flex-col gap-1 ${className}`}>
        ...
        <input  type={type} name = {name} placeholder={placeholder} className={`
          ...
          ${valid==true || valid == null? 'border-white focus:border-primary-blue' : 'border-pink-500 text-pink-600'}
        `} onChange={onChange}/>
