...
export const LocationAwareMap: React.FC<LocationAwareMapProps> = ({
  ...
  const createMapError = useCallback(
    (type: MapErrorType, message: string): MapError => {
      const canRetry = [
        "MAP_LOAD_FAILED",
        "NETWORK_ERROR",
        "GEOCODING_FAILED",
      ].includes(type);
      const fallbackAvailable = ["MAP_LOAD_FAILED", "API_UNAVAILABLE"].includes(
        type,
      );
  ...
  const handleMapLoadTimeout = useCallback(() => {
    const error = createMapError(
      "MAP_LOAD_FAILED",
      "Map is taking too long to load. Please check your internet connection.",
    );
    setMapError(error);
    setMapLoading(false);
  ...
  const checkMapAvailability = useCallback(() => {
    try {
      ...
      const error = createMapError(
        "API_UNAVAILABLE",
        "Map services are currently unavailable. Please try again later.",
      );
      setMapError(error);
  ...
  const ErrorDisplay = () => {
    const displayError =
      ...
      (error
        ? {
            type: "UNKNOWN_ERROR" as MapErrorType,
            message: error,
            canRetry: false,
    ...
    return (
      <div className="absolute top-4 left-4 right-4 z-[1000] bg-red-50 border border-red-200 rounded-lg p-4">
        <div className="flex items-start">
          <div className="flex-shrink-0">{getErrorIcon(displayError.type)}</div>
          <div className="ml-3 flex-1">
            <h3 className="text-sm font-medium text-red-800">
              {displayError.type === "MAP_LOAD_FAILED" && "Map Loading Failed"}
              {displayError.type === "GEOCODING_FAILED" &&
                "Location Services Error"}
              {displayError.type === "NETWORK_ERROR" &&
                "Network Connection Error"}
              {displayError.type === "API_UNAVAILABLE" &&
                "Map Services Unavailable"}
              {displayError.type === "LOCATION_UNAVAILABLE" &&
      ...
      <div className="absolute inset-0 bg-white bg-opacity-90 flex items-center justify-center z-[1000] rounded-lg">
        <div className="flex flex-col items-center space-y-3">
          ...
          <p className="text-xs text-gray-500 text-center max-w-xs">
            {mapLoading &&
              "Setting up interactive map with your location preferences..."}
            {loading &&
              !mapLoading &&
              "Retrieving your saved location settings..."}
          </p>
        </div>
      </div>
    );
      ...
      <div className="border border-gray-200 rounded-lg p-6 bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Interactive map is currently unavailable. You can still set your
            travel preferences below.
          </p>
        ...
        <div className="flex items-center">
          ...
          <div className="ml-3">
            <p className="text-sm text-yellow-800">
              No home location found. Please update your profile to see your
              location on the map.
            </p>
          </div>
        </div>
    ...
    <div className={`space-y-4 ${className}`}>
        ...
        {showFallback ? (
          ...
          >
            ...
            >
              ...
              {homeLocation && (
                <Marker position={homeLocation} icon={createHomeIcon()} />
              )}
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-gray-500">
          Adjust the slider to set your maximum travel distance for job
          opportunities.
          {homeLocation
