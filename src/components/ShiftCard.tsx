...
export default function ShiftCard({
  ...
  return (
    <div
      ...
      className="bg-white flex flex-row items-center justify-between rounded-2xl"
    >
      <div className="w-full bg-white flex flex-col gap-3 rounded-2xl p-3">
        ...
        <div className="flex flex-row gap-2 items-center">
          ...
          <p className="text-xs font-montserrat text-secondary-text">{`${format(
            shift.start_time,
            "hh:mm a"
          )} - ${format(shift.end_time, "hh:mm a")}`}</p>
        </div>
        <div className="flex flex-row gap-2 items-center">
          <img
            className="w-4 h-4"
            src={`${
              shift.staff_assigned == shift.staff_needed
                ...
                : "/icons/userswarning.svg"
            }`}
          />
          <p
            className={`${
              shift.staff_assigned == shift.staff_needed
                ? "text-secondary-text font-montserrat"
                : "text-pink-500 font-montserrat-b"
            } text-xs`}
          >
            {shift.staff_assigned == shift.staff_needed
              ? `${shift.staff_needed}`
